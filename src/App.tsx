  ...
  const generateToken = useCallback(
    async (paymentInitiation) => {
      ...
      if (data) {
        ...
        console.log(`
          Step 2a: Received the link token from the server: ${JSON.stringify(
            data
            )}`
          );
      ...
      <div className={styles.container}>
        ...
        {linkSuccess && isItemAccess && (
          <>
            <Products />
            <Items />
            <button onClick={simpleTransactionalCall}>Get Transaction Data</button>
          </>
        )}
      </div>
    </div>
...
const simpleTransactionalCall = async () => {
  // Call out local server
